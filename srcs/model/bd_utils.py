    ...
    def forward(self, pos):
        if self.pe_embed is False:
            ...
            for i in range(self.levels):
                ...
                temp_value = temp_value.unsqueeze(0)

                pe_list += [torch.sin(temp_value), torch.cos(temp_value)]

            return torch.stack(pe_list, 1)
    # def forward(self, pos, temp_idx):
    #     if self.pe_embed is False:
    #         return pos[:, None]  # 如果不嵌入位置信息，直接返回 pos
    #     else:
    #         pe_list = []
    #         for i in range(1):
    #             temp_value = pos * self.lbase ** (i) * math.pi * temp_idx
    #             pe_list += [torch.sin(temp_value), torch.cos(temp_value)]
                
    #         return torch.stack(pe_list, 1)  # 返回嵌入的位置信息
